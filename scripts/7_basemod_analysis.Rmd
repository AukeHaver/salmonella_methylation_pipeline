...
basemod_distances %>% rownames_to_column("query")%>%
  ...
  ggplot(aes(x=query,y=match,fill=distances))+
  geom_tile()+
  theme_minimal()+
  ...
  theme(axis.text.x=element_text(color="black",angle=45,hjust=1),
        ...
        aspect.ratio = 1)+
  labs(fill="Different Base Modifications",y="Strain",x="Strain")
...
hs_cg_vf_basemods %>%
  ...
  mutate(gene = ifelse(n()>1,
                       paste0(gene,"_",row_number()),
                       gene)) %>%
  ungroup()%>%
  group_by(gene)%>%
  mutate(adjusted_basemod_count = basemod_count/mean(basemod_count))%>%
  ggplot(aes(x=gene,y=Source,fill=log(basemod_count)))+
  geom_tile()+
  scale_fill_viridis_c()+
  labs(fill="Log\n Basemod Count",y="Strain", x="Shared Virulence Factor")+
  theme(axis.text.x=element_text(color="black",angle=90,hjust=1,size=6),
